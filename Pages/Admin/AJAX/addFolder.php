...
if ($parentID == "masterDeck") {
    ...
    //deck added is leaf
    if ($type == 1) {
        $parentDecks = ["('$uuid', '$uuid')"];
        $currentParent = $parentID;

        while ($currentParent) {
            $parentDecks[] = "('$currentParent', '$uuid')";

            $parentQuery = mysqli_query($con, "SELECT parent_deck_id FROM decks WHERE deck_id = '$currentParent'");
            $parentRow = mysqli_fetch_assoc($parentQuery);
            $currentParent = $parentRow["parent_deck_id"];
        }
    ...
    $getAssignedUsers = mysqli_query($con, "SELECT user_id FROM junction_deck_user WHERE deck_id = '$parentID'");
    while ($assignedUser = mysqli_fetch_assoc($getAssignedUsers)) {
        $userId = $assignedUser["user_id"];
        mysqli_query($con, "INSERT INTO junction_deck_user (deck_id, user_id) VALUES ('$uuid', '$userId')");
    }
}
?>
