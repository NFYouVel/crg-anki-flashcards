    ...
    if (isset($_FILES['link'])) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            try {
                ...
                // First pass: collect all IDs and build allLinks
                foreach ($sheet->getRowIterator() as $row) {
                    ...
                    $cardIDsToFetch[] = $cardID;
                    $sentenceCodeToFetch[] = $sentenceCode;
                }

                // Fetch all card data in one query
                $cardCache = [];
                if (!empty($cardIDsToFetch)) {
                    $cardIDList = implode(",", array_map('intval', $cardIDsToFetch));
                    $cardResult = mysqli_query($con, "SELECT card_id, chinese_sc, pinyin, word_class, meaning_eng FROM cards WHERE card_id IN ($cardIDList)");
                    while ($row = mysqli_fetch_array($cardResult)) {
                        $cardCache[$row['card_id']] = $row;
                    }
                }

                // Fetch all sentence data in one query
                $sentenceCache = [];
                if (!empty($sentenceCodeToFetch)) {
                    $sentenceCodeList = implode("','", array_map(function ($c) use ($con) {
                        return mysqli_real_escape_string($con, $c);
                    }, $sentenceCodeToFetch));
                    $sentenceResult = mysqli_query($con, "SELECT sentence_code, chinese_sc FROM example_sentence WHERE sentence_code IN ('$sentenceCodeList')");
                    while ($row = mysqli_fetch_array($sentenceResult)) {
                        $sentenceCache[$row['sentence_code']] = $row;
                    }
                }

                // Second pass: validate using cached data
                foreach ($allLinks as $key => $links) {
                    $cardID = $links["cardID"];
                    $sentenceCode = $links["sentenceCode"];
                    $priority = $links["priority"];

                    $cardInfo = isset($cardCache[$cardID]) ? $cardCache[$cardID] : null;
                    $sentenceInfo = isset($sentenceCache[$sentenceCode]) ? $sentenceCache[$sentenceCode] : null;
        ...
        $_SESSION["cardCache"] = $cardCache ?? [];
        $_SESSION["sentenceCache"] = $sentenceCache ?? [];
        ...
        <div id="tables">
            <table>
                ...
                if (isset($_SESSION["allLinks"])) {
                    $cardCache = $_SESSION["cardCache"] ?? [];
                    $sentenceCache = $_SESSION["sentenceCache"] ?? [];
