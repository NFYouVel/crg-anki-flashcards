...
<head>
    ...
    <script>
        function uploadCards() {
            ...
            xmlhttp.open("GET", "AJAX/batch_junction_deck_card.php?deckID=<?php $deckID = $_GET["deckID"];
                                                                            echo $deckID; ?>", true);
            xmlhttp.send();
        }
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            try {
                ...
                foreach ($sheet->getRowIterator() as $row) {
                    ...
                    try {
                        $priority = $priorityCell->getCalculatedValue();
                    } catch (\Exception $e) {
                        $priority = $rawFormula;
                        echo "<script>console.warn('Formula calc failed on A$index: " . addslashes($e->getMessage()) . "')</script>";
                    }
                    echo "<script>console.log('Row $index -> raw: " . addslashes((string)$rawFormula) . " | calculated: " . addslashes((string)$priority) . "')</script>";
                    ...
                    // ===== DEBUG: guard against non-numeric priority (covers unresolved formulas, formula errors like #REF!, stray text, empty cells) =====
                    if (!is_numeric($priority)) {
                        $reason .= "<p id = 'invalid'>Priority is not a valid number: '" . htmlspecialchars((string)$priority) . "'</p>";
                    }
                    ...
                    $allCards[$cardID] = [
                        ...
                        "fl_priority" => $priority ?? "not found"   // ===== new: value read from the uploaded Excel file =====
                    ];
                    ...
                    if ($reason == "") {
                        ...
                        $invalidCards[$cardID] = [
                            ...
                            "english" => $cardInfo["meaning_eng"] ?? "not found",
                            "indo" => $cardInfo["meaning_ina"] ?? "not found",
                            "fl_priority" => $priority ?? "not found",
