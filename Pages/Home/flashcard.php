...
if ($green !== 0) {
    if ($deckID == "main") {
        ...
        leaf_decks AS (
            SELECT deck_id, name FROM child_decks WHERE is_leaf = 1
        ),
        flashcard AS (
            SELECT c.pinyin, c.chinese_tc, c.chinese_sc, c.word_class, c.meaning_eng, c.meaning_ina, c.card_id, cp.current_stage, cp.review_due, dc.priority, ld.name
            FROM junction_deck_user AS du
        ...
        SELECT * FROM flashcard WHERE review_due <= NOW() ORDER BY name ASC, priority ASC LIMIT 1
        ");
    }
} else {
    if ($deckID == "main") {
        ...
        SELECT * FROM flashcard WHERE total_review = 0 ORDER BY name ASC, priority ASC LIMIT 1
        ");
    }
}
